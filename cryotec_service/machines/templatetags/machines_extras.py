...
@register.filter
def machine_types_astree(mtypes):
    mlist = []
    for mtype in mtypes:
        import logging
        logging.info("hi%s"%mtype)
        mlist.append(mtype)
        mmarks = mtype.machinemark_set.all()
        mmarkslist = [mmark for mmark in mmarks]
        for mmark in mmarks:
            mmarkslist.append(mmark)
            mmarkslist.append([m for m in mmark.machine_set.all()])
        mlist.append(mmarkslist)
    return mlist
    ...
    def render(self, context):
        ...
        def render_item(item, sub_items, level):
            return ''.join([
                ...
                sub_items and '<ul style="diplay:none">%s</ul>' % ''.join(render_items(sub_items, level + 1)) or '',
                '</li>'
            ])
